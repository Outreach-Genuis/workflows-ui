...
const node = {"name":"Create Job for Existing Customer","id":"servicetitan_api_create_job_for_existing_customer","descripition":"Creates a new job for an existing customer in ServiceTitan. Summary is required and will be used to automatically classify the job using GPT. Campaign ID is taken from MCP property.","inputSchema": {
    ...
    jobTypeId: {
      name: "jobTypeId",
      description:
        "The job type ID (will be overridden by GPT classification).",
      required: false,
      validationSchema: /.*/,
      errorMessage: "",
      input: { type: "text" },
    ...
    businessUnitId: {
      name: "businessUnitId",
      description:
        "The business unit ID (will be overridden by GPT classification).",
      required: false,
      validationSchema: /.*/,
      errorMessage: "",
      input: { type: "text" },
    ...
    customerType: {
      name: "customerType",
      description:
        "Customer type (Residential or Commercial). Optional, helps with classification.",
      required: false,
      validationSchema: /.*/,
      errorMessage: "",
      input: { type: "text" },
    ...
    incomingPhoneNumber: {
      ...
      description:
        "Caller's phone number to match campaign (optional, uses default if not provided).",
      required: false,
      validationSchema: /.*/,
      errorMessage: "",
      input: { type: "text" },
    ...
    summary: {
      name: "summary",
      description:
        "Detailed job summary including: what happened during the call, the customer's issue/request, any special instructions, urgency level, and relevant timestamps. This is used for automatic job classification and stored as the job description. Example: 'Customer reported water heater leaking since yesterday evening. Requested emergency service. Prefers afternoon appointment.'",
      required: true,
      validationSchema: /.*/,
      errorMessage: "",
      input: { type: "text" },
    ...
    appointments: {
      name: "appointments",
      description:
        "Array of appointment objects with start, end, technicianIds, arrivalWindowStart, arrivalWindowEnd.",
      required: true,
      validationSchema: /.*/,
      errorMessage: "",
      input: { type: "text" },
    ...
    result: {
      name: "Result",
      description:
        "Output of tool Create Job for Existing Customer from API ServiceTitan API",
      example: "",
