...
const node = {"name":"Get Available Technicians","id":"servicetitan_api_get_available_technicians","descripition":"Retrieves both regular available technicians with their free time slots AND the on-call technician for a specific date.","inputSchema": {
    summary: {
      name: "summary",
      description:
        "Job summary to determine business unit for filtering technicians. Only used when filter_technicians_by_business_unit is enabled.",
      required: false,
      validationSchema: /.*/,
      errorMessage: "",
      input: { type: "text" },
    ...
    result: {
      name: "Result",
      description:
        "Output of tool Get Available Technicians from API ServiceTitan API",
      example: "",
