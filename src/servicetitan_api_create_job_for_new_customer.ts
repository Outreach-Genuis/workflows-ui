...
const node = {"name":"Create Job for New Customer","id":"servicetitan_api_create_job_for_new_customer","descripition":"Creates a new customer FIRST, then creates a job for that customer in ServiceTitan. Two-step process. Summary is required and will be used to automatically classify the job using GPT to determine businessUnitId and jobTypeId. Campaign ID is taken from MCP property.","inputSchema": {
    ...
    summary: {
      name: "summary",
      description:
        "Job summary description. Required for automatic GPT classification.",
      required: true,
      validationSchema: /.*/,
      errorMessage: "",
      input: { type: "text" },
    ...
    customer: {
      name: "customer",
      description:
        "Customer data object with name, type, locations, address, contacts.",
      required: true,
      validationSchema: /.*/,
      errorMessage: "",
      input: { type: "text" },
    ...
    result: {
      name: "Result",
      description:
        "Output of tool Create Job for New Customer from API ServiceTitan API",
      example: "",
