...
const node = {"name":"Wait Time","id":"waittime","descripition":"Pauses the workflow execution for a specified duration before proceeding to the next step","inputSchema": {
    waitType: {
      ...
      parse: function (value: string) {
        return ["duration", "datetime", "specificHour"].includes(value)
          ? (value as "duration" | "datetime" | "specificHour")
          : "duration";
      },
    },
    duration: {
    ...
    timezone: {
      ...
      description:
        "Timezone for the specific hour wait (used when Wait Type is Specific Hour)",
      ...
      errorMessage:
        "Invalid timezone format (e.g., 'America/New_York' or 'UTC')",
      required: false,
      input: {
        type: "select",
        options: [
      ...
      parse: function (value: string): string {
        // Basic validation: check if the value is a non-empty string
  ...
  },"outputSchema": {},"groups":["system","Control Flow","Utility"],"branches":{"success":{"id":"success","name":"Success","description":"Successful execution"}},"isTriggerNode":false,"cost":0} as Node;
