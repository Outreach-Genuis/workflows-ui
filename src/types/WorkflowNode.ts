...
> extends EventEmitter {
  ...
  protected validateOutput(
    ...
    for (const key in this.outputSchema) {
      ...
      if (!field.required && !value) {
        continue; // skip validation for optional fields if not provided
      }
  ...
  public async executeSync(
    rawInput: Record<string, any>,
    runMetadata: RunMetadata,
  ): Promise<InferDataType<Output>> {
    return new Promise<InferDataType<Output>>((resolve, reject) => {
      this.once("done", (response) => {
        if (response.runMetadata.runId === runMetadata.runId) {
      ...
      this.execute(rawInput, runMetadata).catch((error) => {
        reject(error);
      });
    });
  }
